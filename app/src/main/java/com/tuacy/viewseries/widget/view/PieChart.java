...
public class PieChart extends View {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		int height = (int) ((mPieRadius - mRingSelectWidth + mRingNormalWidth / 2 + mMarkerLineLength + textHeight + textPadding + padding) * 2);
	...
	private void drawRing(Canvas canvas) {
		...
		for (PieDataHolder pieDataHolder : pieDataHolders) {
			...
			if (pieDataHolder.mIsSelect) {
				...
				drawRingSection(pieDataHolder.mStartAngel + mRotateAngle, pieDataHolder.mSweepAngel, innerInnerRadius, innerOuterRadius, canvas,
								mPaint);
			} else {
				float innerInnerRadius = mPieRadius - mRingSelectWidth;
	...
	private void drawText(Canvas canvas) {
		...
		for (int index = 0; index < pieDataHolders.size(); index++) {
			...
			final float startX = (float) (getWidth() / 2 + (mPieRadius - mRingSelectWidth + mRingNormalWidth / 2) *
														   Math.cos(Math.toRadians(middle)));
			final float startY = (float) (getHeight() / 2 + (mPieRadius - mRingSelectWidth + mRingNormalWidth / 2) *
															Math.sin(Math.toRadians(middle)));
			canvas.drawCircle(startX, startY, 2, mMarkerLinePaint);
			linePath.moveTo(startX, startY);
			final float x = (float) (getWidth() / 2 + (mMarkerLineLength + mPieRadius - mRingSelectWidth + mRingNormalWidth / 2) * Math.cos(Math.toRadians(middle)));
			final float y = (float) (getHeight() / 2 +
									 (mMarkerLineLength + mPieRadius - mRingSelectWidth + mRingNormalWidth / 2) * Math.sin(Math.toRadians(middle)));
			linePath.lineTo(x, y);
			float landLineX;
